...
**Goal:** Establish one reliable picture of the current repository, runtime, database, RPC, identity, serialization, and deployment state.
...
**Goal:** Editing must be deterministic and visually stable.
...
**Exit evidence:** Runtime editing test with no flicker and no history reset during normal edits.

## Phase 2 — MapDocument & Serialization Contract

**Purpose:** Prevent different systems from interpreting the map differently.

**Goal:** Define one canonical, validated, versioned representation that survives save/load round trips.
...
**Exit evidence:** serialize → parse → compare round-trip tests plus malformed-payload tests.
...
**Exit evidence:** authoritative Save succeeds, produces the expected version, and exposes exact failure information when deliberately broken.
...
**Exit evidence:** Save Slot 1 → edit → Load Slot 1 restores the exact saved document without flicker.

## Phase 5 — Derived Projections

**Purpose:** Make gameplay-facing tables useful without confusing them with editor truth.

**Goal:** Deterministically rebuild projections from authoritative snapshots.

**Must establish:**
- ground/cell projection;
- object projection;
- geometry projection;
...
**Exit evidence:** projections can be deleted/rebuilt and produce the expected state from the same authoritative snapshot.
...
**Goal:** Terrain, seasonal/weather behavior, coordinates, World, and Region context consume stable map contracts.
...
- world/region metadata boundaries.
...
**Exit evidence:** feature-by-feature tests mapped to Game Design and Technical requirements.

## Phase 8 — Verification, Recovery & Release

**Purpose:** Prove that the editor remains reliable outside the happy path.

**Goal:** Release only after recovery, failure, compatibility, and deployment behavior are demonstrated.
...
Phase 1: Is the editor itself stable?
Phase 2: Is the map representation deterministic?
