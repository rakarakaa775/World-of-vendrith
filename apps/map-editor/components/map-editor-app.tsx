...
export function MapEditorApp(){
  ...
  const[showSaveSlots,setShowSaveSlots]=useState(false); const[saveSlots,setSaveSlots]=useState<SaveSlot[]>([]);
  ...
  const refreshSaveSlots=useCallback(async(client:any,mapId:string)=>{const {data,error}=await client.from('map_editor_save_slots').select('slot_number,label,version_number,updated_at').eq('map_id',mapId).order('slot_number',{ascending:true});if(error)throw error;setSaveSlots((data??[]) as SaveSlot[])},[]);
  ...
  const loadFromSlot=useCallback(async(slotNumber:number)=>{const client=createMapEditorSupabaseClient();if(!client||!persistedMapId)return;setBusy(true);setPersistenceStatus(`Loading Save Slot ${slotNumber}…`);try{const {data,error}=await client.from('map_editor_save_slots').select('snapshot,version_number,label').eq('map_id',persistedMapId).eq('slot_number',slotNumber).single();if(error)throw error;if(!data?.snapshot)throw new Error('Save slot is empty');const document=data.snapshot as MapDocument;setMaps([document]);setActiveMapId(document.id);setAuthoritativeVersion(data.version_number??0);setBaseDocument(document);setPersistenceStatus(`Loaded ${data.label||`Save Slot ${slotNumber}`} · version ${data.version_number}`);setShowSaveSlots(false)}catch(error){setPersistenceStatus(`Load Slot ${slotNumber} failed: ${error instanceof Error?error.message:'unknown error'}`)}finally{setBusy(false)}},[persistedMapId]);
  return <div style={{display:'grid',gridTemplateRows:'auto 1fr',height:'100vh'}}><MapBrowser maps={maps} activeMapId={active.id} onMapsChange={setMaps} onOpen={setActiveMapId}/><div style={{position:'relative',minHeight:0}}><div style={{position:'absolute',top:8,right:8,zIndex:10,display:'flex',gap:6,alignItems:'center',padding:6,border:'1px solid #334155',borderRadius:6,background:'#0f172a'}}><button onClick={()=>setShowSaveSlots(true)} disabled={busy||!persistedMapId}>Save / Load</button><button onClick={save} disabled={busy||!persistedMapId||active.id!==persistedMapId}>Quick Save</button><button onClick={load} disabled={busy||!persistedMapId}>Load Latest</button><span style={{fontSize:11,opacity:.8}}>v{authoritativeVersion} · {persistenceStatus}</span></div><EditorShell key={active.id} initialDocument={active} onDocumentChange={update} terrainBindings={terrainBindings} terrainStatus={`${terrainStatus} · ${environmentStatus}`} environmentCatalog={environmentCatalog} environmentReadiness={environmentReadiness} environmentRuntime={environmentRuntime}/>{conflictResult&&conflictSession&&<ConflictResolutionEditorOverlay result={conflictResult} session={conflictSession} onCancel={closeConflictResolution} onResolved={applyConflictResolution}/>}</div><EnvironmentRuntimeBadge worldId={runtimeWorldId} state={environmentRuntime} error={environmentRuntimeError}/><SaveSlotsPanel open={showSaveSlots} slots={saveSlots} busy={busy} onClose={()=>setShowSaveSlots(false)} onSave={saveToSlot} onLoad={loadFromSlot}/></div>;
}
