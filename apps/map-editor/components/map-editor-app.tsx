...
export function MapEditorApp(){
  ...
  const applyConflictResolution=useCallback(async(resolved:MapDocument)=>{if(!conflictResult)return;setBusy(true);try{if(await persistResolved(resolved,conflictResult.conflicts.length>0?authoritativeVersion:authoritativeVersion))closeConflictResolution();}catch(error){setPersistenceStatus(`Conflict commit failed: ${error instanceof Error?error.message:'unknown error'}`)}finally{setBusy(false)}},[conflictResult,persistResolved,closeConflictResolution,authoritativeVersion]);
