...
export function MapEditorApp(){
  ...
  const applyConflictResolution=useCallback((resolved:MapDocument)=>{update(resolved);closeConflictResolution();setPersistenceStatus('Conflict resolved locally; save to persist the resolved map')},[update,closeConflictResolution]);
  useEffect(()=>{
    let cancelled=false;
    ...
    if(persistedMapId){loadMapDocumentSnapshot(client,persistedMapId).then(({result,document})=>{if(cancelled)return;if(document){setMaps([document]);setActiveMapId(document.id);setPersistenceStatus('Saved map loaded')}else setPersistenceStatus(result.code==='MAP_ACCESS_DENIED'?'Map access denied':'No saved snapshot found')}).catch(error=>{if(!cancelled)setPersistenceStatus(`Load failed: ${error instanceof Error?error.message:'unknown error'}`)})}
  ...
  return <div style={{display:'grid',gridTemplateRows:'auto 1fr',height:'100vh'}}>
    ...
    <div style={{position:'relative',minHeight:0}}>
      <div style={{position:'absolute',top:8,right:8,zIndex:10,display:'flex',gap:6,alignItems:'center',padding:6,border:'1px solid #334155',borderRadius:4,background:'#0f172a'}}><button onClick={save} disabled={busy||!persistedMapId||active.id!==persistedMapId}>Save Map</button><button onClick={load} disabled={busy||!persistedMapId}>Load Map</button><span style={{fontSize:11,opacity:.8}}>{persistenceStatus}</span></div>
      ...
      {conflictResult&&conflictSession&&<ConflictResolutionEditorOverlay result={conflictResult} session={conflictSession} onCancel={closeConflictResolution} onResolved={applyConflictResolution}/>} 
    </div>
