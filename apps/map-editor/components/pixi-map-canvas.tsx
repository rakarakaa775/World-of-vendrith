...
function drawBrushPreview(preview: Graphics, document: MapDocument, layerId: string, points: GridPoint[], selectedTileId: string | null, t: number, activeTool: string, terrainBindings: TerrainAssetBindingMap) {
  ...
  for (const cell of analysis.cells) {
    ...
    preview.rect(x*t+2,y*t+2,t-4,t-4).fill({color,alpha}).stroke({width:1,color:0x334155,alpha:0.75});
...
export function PixiMapCanvas({document,activeTool,activeLayerId,selectedTileId,brushSize,selection,onPaint,onSelectionChange,onCellInspect,onStamp,onObjectPlace,onObjectMove,selectedObjectId,terrainBindings={},environmentRuntime=null}:Props){
  ...
  useEffect(()=>{let disposed=false;let initialized=false;let destroyed=false;let runtimeDisposed=false;let removeWheel:(()=>void)|null=null;const host=hostRef.current;if(!host)return;const runtime=createTerrainRuntime(terrainBindings);const app=new Application();
    const disposeRuntime=()=>{if(runtimeDisposed)return;runtimeDisposed=true;runtime.dispose();};
    const destroyApp=()=>{if(destroyed)return;destroyed=true;try{app.destroy({removeView:true,releaseGlobalResources:false},{children:true});}catch(error){console.error('Pixi application destroy failed',error);}};
    void app.init({resizeTo:host,background:'#ffffff',antialias:true}).then(()=>{if(disposed){destroyApp();disposeRuntime();return;}initialized=true;host.replaceChildren(app.canvas);const world=new Container();const grid=new Graphics();const overlay=new Graphics();const preview=new Graphics();const width=document.width*document.tileSize;const height=document.height*document.tileSize;const terrainPlans=new Map<string,ReturnType<typeof buildTerrainRenderPlan>>();
      grid.rect(0,0,width,height).stroke({width:1,color:0x64748b});for(let x=1;x<document.width;x++)grid.moveTo(x*document.tileSize,0).lineTo(x*document.tileSize,height);for(let y=1;y<document.height;y++)grid.moveTo(0,y*document.tileSize).lineTo(width,y*document.tileSize);grid.stroke({width:1,color:0xcbd5e1});world.addChild(grid);
      ...
      app.stage.eventMode='static';app.stage.hitArea=app.screen;app.stage.on('pointerdown',down).on('pointermove',move).on('pointerup',up).on('pointerupoutside',up);host.addEventListener('wheel',wheel,{passive:true});removeWheel=()=>host.removeEventListener('wheel',wheel);void Promise.all(textureJobs);
    }).catch(error=>{if(!disposed)console.error('Pixi map canvas initialization failed',error);});
    return()=>{disposed=true;removeWheel?.();removeWheel=null;disposeRuntime();if(initialized)destroyApp();};
  ...
  return <div ref={hostRef} style={{width:'100%',height:'100%',minHeight:320,overflow:'hidden',background:'#ffffff'}}/>;
}
