  ...
  useEffect(()=>{let disposed=false;let initialized=false;let destroyed=false;let runtimeDisposed=false;let removeWheel:(()=>void)|null=null;const host=hostRef.current;if(!host)return;const runtime=createTerrainRuntime(terrainBindings);const app=new Application();
    ...
    void app.init({resizeTo:host,background:'#0f1318',antialias:true}).then(()=>{if(disposed){destroyApp();disposeRuntime();return;}initialized=true;host.replaceChildren(app.canvas);const world=new Container();const grid=new Graphics();const overlay=new Graphics();const preview=new Graphics();const width=document.width*document.tileSize;const height=document.height*document.tileSize;const terrainPlans=new Map<string,ReturnType<typeof buildTerrainRenderPlan>>();
      ...
      for(const layer of document.layers){if(!layer.visible)continue;const terrainPlan=layer.kind==='ground'?terrainPlans.get(layer.id):null;for(let i=0;i<layer.cells.length;i++){const id=layer.cells[i]?.tileId;if(!id)continue;const x=i%document.width,y=Math.floor(i/document.width),t=document.tileSize,tile=new Graphics();tile.rect(x*t+2,y*t+2,t-4,t-4).fill({color:tileColor(id,layer.kind),alpha:layer.kind==='collision'?0.32:1});
        ...
        if(layer.kind==='objects')for(const o of layer.objects){const g=new Graphics();const objectColor=o.category==='house'?0x8b5e3c:o.category==='tree'?0x3f7d45:0x777777;g.rect(o.x*document.tileSize+3,o.y*document.tileSize+3,o.width*document.tileSize-6,o.height*document.tileSize-6).fill({color:objectColor,alpha:0.9}).stroke({width:2,color:selectedObjectId===o.id?0xf8fafc:0x111827});world.addChild(g);const label=new Text({text:o.category[0]?.toUpperCase()??'O',style:{fontSize:14,fill:0xffffff}});label.position.set(o.x*document.tileSize+10,o.y*document.tileSize+8);world.addChild(label);}
      }
