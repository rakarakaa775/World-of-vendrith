...
import { WorldBrowser } from "./world-browser";
...
export type WorldBuilderStartMode = "load" | "create";

export function VendrithWorldBuilderApp({ startMode = "load" }: { startMode?: WorldBuilderStartMode }) {
  ...
  const bootstrap = useCallback(async (client: any, doc: MapDocument, mapId: string) => {
    ...
    const raw = await p.commitResolvedMerge(mapId, 0, serializeResolvedMapSnapshot(doc), "world-builder-bootstrap-v4");
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (!session.data.session) {
        throw new Error("AUTH_REQUIRED: Please sign in before opening the Vendrith World Builder.");
      }
      if (cancelled) return;
