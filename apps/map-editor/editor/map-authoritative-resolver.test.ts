...
const clientFor = (row: Record<string, unknown>, snapshot: unknown, access = true, identityMapType = row.map_type ?? 'world') => ({
  rpc: vi.fn(async (name: string) => {
    if (name === 'map_editor_resolve_identity_v1') return access
      ? { data: [{ editor_map_id: row.editor_map_id ?? row.id ?? 'world-1', legacy_map_id: row.legacy_map_id ?? row.id ?? 'world-1', world_id: row.world_id ?? 'world-id', map_type: identityMapType, parent_editor_map_id: null, created_by: 'owner-1' }], error: null }
