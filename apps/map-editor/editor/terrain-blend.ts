...
export function terrainBlendsForMask(document: MapDocument, layerId: string, point: GridPoint): TerrainBlend[] {
  ...
  if (!terrain) return [];
  const mask = neighborMask(document, layerId, point, terrain);
  return DIRECTIONS.flatMap(([direction, dx, dy, bit, kind]) => {
    if ((mask & bit) !== 0) return [];
    const x = point.x + dx, y = point.y + dy;
    if (x < 0 || y < 0 || x >= document.width || y >= document.height) return [];
    const neighbor = terrainAt(document, { x, y }, layerId);
    if (!neighbor || neighbor === terrain) return [];
    return [{ point, terrain, direction, neighborTerrain: neighbor, mask, key: terrainBlendKey(terrain, neighbor, direction), kind }];
  });
}
