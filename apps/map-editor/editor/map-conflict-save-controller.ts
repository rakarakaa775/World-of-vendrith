...
export type ConflictSaveResult =
  ...
  | { status: 'conflict'; merge: MapMergeResult; expectedVersion: number; remoteVersion: number }
...
export async function saveWithConflictDetection(
  ...
  try {
    ...
    if (remoteVersion !== expectedVersion || merge.conflicts.length > 0) {
      return { status: 'conflict', merge, expectedVersion, remoteVersion };
    }

    const persistence = createSupabaseMapMergePersistence(client);
    const committed = await persistence.commitResolvedMerge(
      local.id,
      expectedVersion,
    ...
    if (committed.status === 'conflict') {
      ...
      return {
        ...
        expectedVersion,
        remoteVersion: refreshed.result.version_number ?? committed.current_version,
      };
    }
