...
export function pointsInFloodFill(document:MapDocument,layerId:string,start:GridPoint):GridPoint[]{if(start.x<0||start.y<0||start.x>=document.width||start.y>=document.height)return[];const layer=document.layers.find(item=>item.id===layerId);if(!layer)return[];const target=layer.cells[start.y*document.width+start.x]?.tileId??null;const queue=[start],seen=new Set<string>([pointKey(start)]),result:GridPoint[]=[];while(queue.length){const point=queue.shift()!;result.push(point);for(const next of[{x:point.x+1,y:point.y},{x:point.x-1,y:point.y},{x:point.x,y:point.y+1},{x:point.x,y:point.y-1}]){if(next.x<0||next.y<0||next.x>=document.width||next.y>=document.height)continue;const key=pointKey(next);if(seen.has(key))continue;if((layer.cells[next.y*document.width+next.x]?.tileId??null)!==target)continue;seen.add(key);queue.push(next);}}return result;}

export function pointsForPaintShape(document:MapDocument,layerId:string,shape:PaintShape,start:GridPoint,end=start,size=1):GridPoint[]{switch(shape){case'square':return pointsInSquare(end,size);case'line':return pointsInLine(start,end);case'rectangle':return pointsInRectangle(start,end);case'flood':return pointsInFloodFill(document,layerId,start);}}
