...
export function diffMapDocuments(previous: MapDocument | null, next: MapDocument): MapRenderDiff {
  if (!previous || previous.width !== next.width || previous.height !== next.height || previous.tileSize !== next.tileSize) {
    return {
      ...
      changedTerrainByLayer: next.layers.filter(layer => layer.kind !== "objects").map(layer => ({ layerId: layer.id, changedCells: allCells(next.width, next.height) })),
  ...
  const layerStructureChanged =
    previous.layers.length !== next.layers.length ||
    previous.layers.some((layer, index) => {
      const nextLayer = next.layers[index];
      return !nextLayer || layer.id !== nextLayer.id || layer.kind !== nextLayer.kind || layer.visible !== nextLayer.visible || layer.locked !== nextLayer.locked;
    });
    ...
    if (!previousLayer || previousLayer.kind === "objects") {
      ...
      for (let index = 0; index < length; index++) {
        if (previousLayer.cells[index]?.tileId !== nextLayer.cells[index]?.tileId) {
          changedCells.push({ x: index % next.width, y: Math.floor(index / next.width) });
        }
      }
    }
    if (changedCells.length > 0) {
      const uniqueChanged = [...new Map(changedCells.map(point => [pointKey(point), point])).values()];
      changedTerrainByLayer.push({ layerId: nextLayer.id, changedCells: affectedTerrainCells(next, uniqueChanged) });
    }
  }

  const previousObjects = new Map((previous.layers.find(layer => layer.kind === "objects")?.objects ?? []).map(object => [object.id, object]));
  const nextObjects = new Map((next.layers.find(layer => layer.kind === "objects")?.objects ?? []).map(object => [object.id, object]));
  ...
  for (const id of previousObjects.keys()) if (!nextObjects.has(id)) changedObjectIds.add(id);
  ...
  const objectLayerChanged = changedObjectIds.size > 0 || (previousObjectLayer?.visible ?? true) !== (nextObjectLayer?.visible ?? true);

  return { dimensionsChanged: false, changedTerrainByLayer, changedObjectIds: [...changedObjectIds], objectLayerChanged, layerStructureChanged };
}
